...
public class VncViewer extends java.applet.Applet implements
    ...
    KeyboardEvent.extended_key_event = false;
    setPC105();
    //setJP106();
		refApplet = this;
	...
	void setEncodings(boolean autoSelectOnly) {
		...
		encodings[nEncodings++] = RfbProto.EncodingExtendedKeyEvent;
	...
	void readParameters() {
		...
		str = readParameter("DebugEvent", false);
    if (str != null && str.equalsIgnoreCase("Yes")) {
      KeyboardEvent.debug_event = true;
    }
	...
	public void sendCtrlAltDel() {
		try {
			if (rfb != null && !rfb.closed() && vc != null) {
				final int modifiers = InputEvent.CTRL_MASK
						| InputEvent.ALT_MASK;

				KeyEvent ctrlAltDelEvent = new KeyEvent(vc,
						KeyEvent.KEY_PRESSED, 0, modifiers, 0x7f,
						KeyEvent.CHAR_UNDEFINED);
				rfb.writeKeyEvent(ctrlAltDelEvent);

				ctrlAltDelEvent = new KeyEvent(vc, KeyEvent.KEY_RELEASED, 0,
						modifiers, 127, KeyEvent.CHAR_UNDEFINED);
				rfb.writeKeyEvent(ctrlAltDelEvent);
			}
	...
	public void setJP106() {
	  KeyboardEvent.extended_layout = KeyboardEvent.LAYOUT_JP106;
	  System.out.println("Set LAYOUT_JP106");
	}
	
	public void setPC105() {
	   KeyboardEvent.extended_layout = KeyboardEvent.LAYOUT_PC105;
	   System.out.println("Set LAYOUT_PC105");
	}
		...
		try {
			if (rfb != null && !rfb.closed()) {
				rfb.writeFramebufferUpdateRequest(0, 0,
						rfb.framebufferWidth == 0 ? 640 : rfb.framebufferWidth,
						rfb.framebufferHeight == 0 ? 480
	...
	public int getBorderWidth() {
		if (desktopScrollPane == null) {
			...
			Insets in = desktopScrollPane.getInsets();
			return Math.max(Math.min(in.top, in.bottom), Math.min(in.left,
					in.right));
		}
	}
	...
	public int getScrollbarHeight() {
		if (desktopScrollPane == null) {
			...
			return desktopScrollPane.getHScrollbarHeight();
		}
	}
	...
	public int getScrollbarWidth() {
		if (desktopScrollPane == null) {
			...
			return desktopScrollPane.getVScrollbarWidth();
		}
	}

	public void sendSysRq(String command_) {
		try {
			if (rfb != null && !rfb.closed() && vc != null
					&& !command_.isEmpty()) {
				final char command = (char) Integer.parseInt(command_);
				final char control = 0xff15;
				final int modifiers = InputEvent.ALT_MASK;
				KeyEvent e = new KeyEvent(vc, KeyEvent.KEY_PRESSED, 0,
						modifiers, KeyEvent.VK_UNDEFINED, control);
				rfb.writeKeyEvent(e);
				e = new KeyEvent(vc, KeyEvent.KEY_PRESSED, 0, modifiers,
						KeyEvent.VK_UNDEFINED, command);
				rfb.writeKeyEvent(e);
				e = new KeyEvent(vc, KeyEvent.KEY_RELEASED, 0, modifiers,
						KeyEvent.VK_UNDEFINED, command);
				rfb.writeKeyEvent(e);
				e = new KeyEvent(vc, KeyEvent.KEY_RELEASED, 0, modifiers,
						KeyEvent.VK_UNDEFINED, control);
				rfb.writeKeyEvent(e);
			}
